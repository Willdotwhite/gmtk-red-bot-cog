...
class JamCommands(commands.Cog):
    ...
    def __init__(self, bot: commands.Bot):
        ...
        self.last_slash_command_posted = {}
    ...
    def _was_last_message_too_recent(self, channel_id: int, command: str) -> bool:
        key = (channel_id, command)
        now = datetime.now(timezone.utc)

        last_posted = self.last_slash_command_posted.get(key)

        if last_posted and (now - last_posted) < timedelta(seconds=15):
            return True

        self.last_slash_command_posted[key] = now
        return False
    ...
    async def cmd_ai(self, interaction: discord.Interaction):
        if self._was_last_message_too_recent(interaction.channel.id, "ai"):
            return
    ...
    async def cmd_jaminfo(self, interaction: discord.Interaction):
        if self._was_last_message_too_recent(interaction.channel.id, "jaminfo"):
            return
        if await self._check_permissions(interaction):
    ...
    async def cmd_jamruling(self, interaction: discord.Interaction):
        if self._was_last_message_too_recent(interaction.channel.id, "jamruling"):
            return
        if await self._check_permissions(interaction):
    ...
    async def cmd_itch(self, interaction: discord.Interaction):
        if self._was_last_message_too_recent(interaction.channel.id, "itch"):
            return
        if await self._check_permissions(interaction):
    ...
    async def cmd_hype(self, interaction: discord.Interaction):
        if self._was_last_message_too_recent(interaction.channel.id, "hype"):
            return
        if await self._check_permissions(interaction):
    ...
    async def cmd_teamfinder(self, interaction: discord.Interaction):
        if self._was_last_message_too_recent(interaction.channel.id, "teamfinder"):
            return
        if await self._check_permissions(interaction):
    ...
    async def cmd_theme(self, interaction: discord.Interaction):
        if self._was_last_message_too_recent(interaction.channel.id, "theme"):
            return
        if await self._check_permissions(interaction):
    ...
    async def cmd_health(self, interaction: discord.Interaction):
        if self._was_last_message_too_recent(interaction.channel.id, "health"):
            return
        if await self._check_permissions(interaction):
    ...
    async def cmd_jamtips(self, interaction: discord.Interaction):
        if self._was_last_message_too_recent(interaction.channel.id, "jamtips"):
            return
        if await self._check_permissions(interaction):
    ...
    async def cmd_jamstart(self, interaction: discord.Interaction):
        if self._was_last_message_too_recent(interaction.channel.id, "jamstart"):
            return
        if await self._check_permissions(interaction):
    ...
    async def cmd_extension(self, interaction: discord.Interaction):
        if self._was_last_message_too_recent(interaction.channel.id, "extension"):
            return
        if await self._check_permissions(interaction):
    ...
    async def cmd_jamend(self, interaction: discord.Interaction):
        if self._was_last_message_too_recent(interaction.channel.id, "jamend"):
            return
        if await self._check_permissions(interaction):
    ...
    async def cmd_build(self, interaction: discord.Interaction):
        if self._was_last_message_too_recent(interaction.channel.id, "build"):
            return
        if await self._check_permissions(interaction):
    ...
    async def cmd_extraextension(self, interaction: discord.Interaction):
        if self._was_last_message_too_recent(interaction.channel.id, "extraextension"):
            return
        if await self._check_permissions(interaction):
    ...
    async def cmd_gamelink(self, interaction: discord.Interaction):
        if self._was_last_message_too_recent(interaction.channel.id, "gamelink"):
            return
        if await self._check_permissions(interaction):
    ...
    async def cmd_jamover(self, interaction: discord.Interaction):
        if self._was_last_message_too_recent(interaction.channel.id, "jamover"):
            return
        if await self._check_permissions(interaction):
    ...
    async def cmd_closing(self, interaction: discord.Interaction):
        if self._was_last_message_too_recent(interaction.channel.id, "closing"):
            return
        if await self._check_permissions(interaction):
    ...
    async def cmd_r4r(self, interaction: discord.Interaction):
        if self._was_last_message_too_recent(interaction.channel.id, "r4r"):
            return
        if await self._check_permissions(interaction):
